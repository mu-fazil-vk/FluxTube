...
import 'package:fluxtube/presentation/watch/widgets/explode/related_video_section.dart';
...
class ExplodeContentScreen extends StatelessWidget {
  final String id;
  final bool isSaved;
            ...
            child: Column(
              ...
              children: [
                if (watchState.isLoading ||
                    watchState.isSubtitleLoading ||
                    watchState.isMuxedStreamsLoading)
...
Widget _buildCaptionRow({
  ...
  required ExplodeWatchResp watchInfo,
  required S locals,
}) {
  return GestureDetector(
    ...
    child: CaptionRowWidget(
      caption: watchState.isLoading ? watchState.title : watchInfo.title,
...
Widget _buildViewRow(ExplodeWatchResp watchInfo) {
...
Widget _buildChannelInfoSection({
  ...
  return ExplodeChannelInfoSection(
    state: watchState,
    watchInfo: watchInfo,
    locals: locals,
  );
...
Widget _buildDescriptionOrRelatedVideos({
  ...
  if (watchState.isDescriptionTapped) {
    return ExplodeDescriptionSection(
      height: screenHeight,
      watchInfo: watchInfo,
      locals: locals,
    );
  ...
  if (!watchState.isTapComments) {
    ...
    return watchState.isRelatedVideosLoading
        ? SizedBox(
            height: 350,
        ...
        : ExplodeRelatedVideoSection(
            locals: locals,
            watchInfo: watchInfo,
            related: watchState.relatedVideos ?? [],
          );
  }
